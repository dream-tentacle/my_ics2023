...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)         \
  ...
  INSTPAT(
      ...
      R(rd) = (sword_t)(((int64_t)((sword_t)src1) * (int64_t)((sword_t)src2)) >>
                        32));
  INSTPAT(
      "0000001 ????? ????? 010 ????? 01100 11", mulhsu, R,
      R(rd) = (sword_t)(((int64_t)((sword_t)src1) * (uint64_t)(src2)) >> 32));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, Zicsr,
          R(rd) = csr_read(imm);
          csr_write(imm, src1));
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, N, printf("123");
          isa_raise_intr(1, s->snpc));
